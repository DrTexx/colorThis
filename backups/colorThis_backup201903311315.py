...
try:
    ...
    print("Can't import colorama. Maybe it isn't installed? Maybe you're running 64-bit instead of 32-bit?")
...
def ct(my_string,**kwargs): # define main function
    ...
    # list of classes from colorama that are compatible by this function
    keywords = ['Back','Fore','Style']
    ...
    if (coloramaImported):
        ...
        try:
            for kwarg in kwargs: # for each kwarg, do this
                ...
                if kwarg in keywords:
                    ...
                    if(debug): print('{:-<20}: {} is not a compatible styling class in colorama; will not be added to string'.format('class',kwarg))
            ...
            if not (my_string == ""): # If my_string isn't empty
                if(debug): print("{:-<16}: string isn't empty, appending it".format('my_string',my_string))
                tempString.append(str(my_string)) # append the inputed string to the output list
            else:
                if(debug): print("looks like the main string is empty? nothing appended.")
        ...
        if(showErrors): print("colorama could not be imported...")
        if(showErrors): print("returning a normal string...")
        return(my_string)

import time
# Print the string without showing errors
print(ct("hello",Back="RED",Fore="WHITE",showErrors=False))
print()
time.sleep(1)

# Print the string with default settings
print(ct("hello",Back="RED",Fore="WHITE"))
print()
time.sleep(1)

# Print the string with debugging active
print(ct("hello",Back="RED",Fore="WHITE",debug=True))

print("coloramaImported value:",str(coloramaImported))
